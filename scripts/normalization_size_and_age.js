...
function normalizeSize(rawSize) {
  ...
  if (
    size === '4' ||
    size.includes('12m') ||
    size.includes('12 month') ||
    size.includes('1 year')
  ) {
    return '12M';
  }
  ...
  if (
    size === '6' ||
    size.includes('24m') ||
    size.includes('24 month') ||
    size.includes('2 years')
  ) {
    return '24M';
  }
...
function normalizeAgeRange(rawAge) {
  ...
  if (
    ...
    age.includes('18 month') ||
    age.includes('24 month')
  ) {
    return '1-2Y';
  }
  ...
  if (
    age.includes('6-12') ||
    age.includes('6 month') ||
    age.includes('9 month') ||
    age.includes('12 month') ||
    age.includes('1 year')
  ) {
    return '6-12M';
  }
  ...
  try {
    ...
    if (needsManualReview.length > 0) {
      ...
      needsManualReview.forEach((item) => {
        console.log(
          `- "${item.title}": ${item.field} = "${item.current}" (ID: ${item.id})`,
        );
      });
    }
  } catch (error) {
