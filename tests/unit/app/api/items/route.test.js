...
jest.mock('@/lib/mongodb', () => ({
  ...
  describe('GET /api/items', () => {
    ...
    it('should filter by category', async () => {
      const request = new Request('http://localhost:3000/api/items?category=clothing');
      await GET(request);
    ...
    it('should filter by condition', async () => {
      const request = new Request('http://localhost:3000/api/items?condition=new');
      await GET(request);
    ...
    it('should filter by size', async () => {
      const request = new Request('http://localhost:3000/api/items?size=0-3%20months');
      await GET(request);
    ...
    it('should filter by ageRange', async () => {
      const request = new Request('http://localhost:3000/api/items?ageRange=0-3%20months');
      await GET(request);
  
      expect(mockCollection.find).toHaveBeenCalledWith(
        expect.objectContaining({
          ageRange: { $regex: expect.stringContaining('0-3 months'), $options: 'i' },
        })
      );
    });
  
    it('should filter by price range', async () => {
      const request = new Request('http://localhost:3000/api/items?priceMin=10&priceMax=20');
      await GET(request);
    ...
    it('should search by searchTerm', async () => {
      const request = new Request('http://localhost:3000/api/items?searchTerm=onesie');
      await GET(request);
    ...
    it('should sort by oldest', async () => {
      const request = new Request('http://localhost:3000/api/items?sortBy=oldest');
      const mockSort = jest.fn(() => ({
        limit: jest.fn(() => ({
          toArray: jest.fn().mockResolvedValue(mockItems),
        })),
    ...
    it('should sort by price low to high', async () => {
      const request = new Request('http://localhost:3000/api/items?sortBy=price-low');
      const mockSort = jest.fn(() => ({
        limit: jest.fn(() => ({
          toArray: jest.fn().mockResolvedValue(mockItems),
        })),
    ...
    it('should sort by price high to low', async () => {
      const request = new Request('http://localhost:3000/api/items?sortBy=price-high');
      const mockSort = jest.fn(() => ({
        limit: jest.fn(() => ({
          toArray: jest.fn().mockResolvedValue(mockItems),
        })),
    ...
    it('should combine multiple filters', async () => {
      const request = new Request('http://localhost:3000/api/items?category=clothing&condition=new&priceMin=10&priceMax=20');
      await GET(request);
