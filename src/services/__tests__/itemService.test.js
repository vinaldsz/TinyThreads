...
describe('itemService', () => {
  ...
  describe('normalizeItem', () => {
    ...
    test('handles item with regular id field', async () => {
      ...
      expect(item.price).toBe(15.50) // Should convert string to number
    })
  ...
  describe('apiFetch error handling', () => {
    test('throws error on non-ok response', async () => {
      ...
      await expect(getItemById('nonexistent')).rejects.toThrow('API 404: Item not found')
    })
    ...
    test('handles response with no error text', async () => {
      ...
      await expect(getItemById('test')).rejects.toThrow('API 500: Internal Server Error')
    })
    ...
    test('handles API response with different error format', async () => {
      ...
      await expect(getItemById('missing')).rejects.toThrow('API 404: Resource not found')
    })
  })
  ...
  describe('getItemById', () => {
    test('fetches single item successfully', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith('/api/items/test-id', expect.any(Object))
    ...
    test('throws error when item not found', async () => {
      fetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ item: null }) // This will make item = null || { item: null } = { item: null }, but normalizeItem(null) should handle this
      })
      
      // Actually, let's test the real error condition - when the API returns an empty response
      fetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ message: "Not found" }) // No 'item' property, no direct data that makes sense
      })
      
      await expect(getItemById('nonexistent')).rejects.toThrow('Item with id nonexistent not found')
    })
  })
