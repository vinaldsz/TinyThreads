...
describe('awss3', () => {
  ...
  describe('environment validation', () => {
    test('throws error when AWS_REGION is missing', async () => {
      ...
      }).rejects.toThrow('Missing AWS credentials or region in environment variables')
    })
  ...
  describe('getPublicUrl', () => {
    ...
    test('handles keys with special characters', () => {
      ...
      expect(result).toBe('https://test-bucket.s3.amazonaws.com/items/item with spaces & symbols!.jpg')
    })
  ...
  describe('uploadImageToS3', () => {
    ...
    test('uploads image successfully with default options', async () => {
      ...
      expect(result).toEqual({
        key: 'items/item-12345678-1234-5678-9012-123456789abc.jpg',
        imageUrl: 'https://test-bucket.s3.amazonaws.com/items/item-12345678-1234-5678-9012-123456789abc.jpg'
      })
    })
    ...
    test('uploads image with custom folder and prefix', async () => {
      ...
      expect(result.key).toBe('profile/avatar-12345678-1234-5678-9012-123456789abc.jpg')
    })
    ...
    test('handles file without ContentType', async () => {
      ...
      await uploadImageToS3(noTypeFile)
    ...
    test('throws error when file is null', async () => {
      await expect(uploadImageToS3(null)).rejects.toThrow('uploadImageToS3: file is required')
      
      expect(mockSend).not.toHaveBeenCalled()
    })

    test('throws error when file is undefined', async () => {
      await expect(uploadImageToS3(undefined)).rejects.toThrow('uploadImageToS3: file is required')
      
      expect(mockSend).not.toHaveBeenCalled()
    })

    test('throws error when file is string', async () => {
      await expect(uploadImageToS3('not-a-file')).rejects.toThrow('uploadImageToS3: file is required')
    ...
    test('throws error when file is not an image', async () => {
      ...
      await expect(uploadImageToS3(textFile)).rejects.toThrow('Only image uploads are allowed')
    ...
    test('throws error when file type is undefined', async () => {
      ...
      await expect(uploadImageToS3(noTypeFile)).rejects.toThrow('Only image uploads are allowed')
    })
    ...
    test('handles S3 upload failure', async () => {
      ...
      await expect(uploadImageToS3(mockFile)).rejects.toThrow('S3 upload failed')
    ...
    test('handles arrayBuffer conversion failure', async () => {
      const fileWithBadArrayBuffer = {
        ...mockFile,
        arrayBuffer: jest.fn().mockRejectedValue(new Error('ArrayBuffer failed'))
      }
      
      await expect(uploadImageToS3(fileWithBadArrayBuffer)).rejects.toThrow('ArrayBuffer failed')
