...
export const ITEM_CONFIG = {
    ...
    AGE_RANGES: ['0-3 months', '3-6 months', '6-12 months', '12-18 months', '18-24 months', '2-3 years', '3+ years'],
  ...
  export const ITEM_SCHEMA = {
    ...
    category: { required: false, type: 'string', values: ITEM_CONFIG.CATEGORIES, default: ITEM_CONFIG.DEFAULTS.category },
    ageRange: { required: true, type: 'string', values: ITEM_CONFIG.AGE_RANGES },
    createdAt: { required: false, type: 'string', default: ITEM_CONFIG.DEFAULTS.createdAt }
  };
  ...
  export function validateItem(item) {
    ...
    Object.keys(ITEM_SCHEMA).forEach(field => {
      ...
      if (value && rules.values && !rules.values.includes(value)) {
        errors.push(`Invalid value for ${field}: ${value}. Must be one of: ${rules.values.join(', ')}`);
      }
    });
