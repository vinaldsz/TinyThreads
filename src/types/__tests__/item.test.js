  ...
  describe('item.js types and utilities', () => {
    ...
    describe('ITEM_CONFIG', () => {
      test('has all required condition values', () => {
        expect(ITEM_CONFIG.CONDITIONS).toEqual(['New', 'Like New', 'Good', 'Fair'])
        expect(ITEM_CONFIG.CONDITIONS).toHaveLength(4)
      })
  
      test('has all required category values', () => {
        expect(ITEM_CONFIG.CATEGORIES).toEqual(['Clothing', 'Toys', 'Books', 'Gear', 'Other'])
    ...
    describe('ITEM_SCHEMA', () => {
      test('has all expected fields', () => {
        const expectedFields = [
          'id', 'title', 'price', 'size', 'condition', 'imageUrl', 
          'description', 'sellerId', 'sellerName', 'category', 'ageRange', 'createdAt'
        ]
      ...
      test('required fields are correctly marked', () => {
        const requiredFields = ['id', 'title', 'price', 'size', 'condition', 'imageUrl', 'description', 'sellerId', 'sellerName', 'ageRange']
      ...
      test('default values are properly referenced', () => {
        ...
        expect(ITEM_SCHEMA.createdAt.default).toBe(ITEM_CONFIG.DEFAULTS.createdAt)
      })
    })
    ...
    describe('validateItem', () => {
      ...
      test('catches invalid condition values', () => {
        ...
        expect(errors).toContain('Invalid value for condition: Invalid Condition. Must be one of: New, Like New, Good, Fair')
      })
      ...
      test('catches invalid category values', () => {
        ...
        expect(errors).toContain('Invalid value for category: Invalid Category. Must be one of: Clothing, Toys, Books, Gear, Other')
      })
      ...
      test('catches invalid age range values', () => {
        ...
        expect(errors).toContain('Invalid value for ageRange: Invalid Age Range. Must be one of: 0-3 months, 3-6 months, 6-12 months, 12-18 months, 18-24 months, 2-3 years, 3+ years')
      })
      ...
      test('catches multiple validation errors', () => {
        ...
        expect(errors).toContain('Missing required field: title')
        expect(errors).toContain('Missing required field: price')
        expect(errors).toContain(expect.stringContaining('Invalid value for condition'))
        expect(errors).toContain(expect.stringContaining('Invalid value for category'))
        expect(errors).toContain(expect.stringContaining('Invalid value for ageRange'))
      })
    ...
    describe('getFilterOptions', () => {
      ...
      test('returns expected structure for UI consumption', () => {
        ...
        expect(Object.keys(options)).toEqual(['conditions', 'categories', 'ageRanges'])
    ...
    describe('exampleItem', () => {
      ...
      test('has all required fields', () => {
        ...
        expect(exampleItem.description).toBe('Soft organic cotton onesie, barely worn')
      ...
      test('has default values applied', () => {
        ...
        expect(new Date(exampleItem.createdAt).toISOString()).toBe(exampleItem.createdAt)
      })
    ...
    describe('integration scenarios', () => {
      ...
      test('all config conditions are valid in schema', () => {
        ITEM_CONFIG.CONDITIONS.forEach(condition => {
          ...
          const conditionErrors = errors.filter(error => error.includes('condition'))
      ...
      test('all config categories are valid in schema', () => {
        ITEM_CONFIG.CATEGORIES.forEach(category => {
          ...
          const errors = validateItem(testItem)
          const categoryErrors = errors.filter(error => error.includes('category'))
      ...
      test('all config age ranges are valid in schema', () => {
        ITEM_CONFIG.AGE_RANGES.forEach(ageRange => {
          ...
          const errors = validateItem(testItem)
          const ageRangeErrors = errors.filter(error => error.includes('ageRange'))
